...
import NextPrevBlocl from "./navigBlockinBlog/NextPrevBlocl";
import "./Blogs.css";
    ...
    <div>
      <div className="Blogs">
        ...
        <div className="BlogsMenu__select">
          ...
          />
          <MyModal visible={modalAddBlog} setVisible={setModalAddBlog}>
            <InnerFormBlog
        ...
        {post.length > 0 ? (
          post.map((item) => (
            <Form post={item} key={Math.random()} remove={remove} />
          ))
        ) : (
          <h1 className="noBlogsYet">No blogs yet</h1>
        )}
      </div>
          <NextPrevBlocl iterPage={iterPage} serIterPage={serIterPage}/>
